...
export function createClassroom1() {
    const elements = new THREE.Group();
    elements.userData.type = "room";
    elements.userData.roomType = "Classroom1";
    ...
    */
    ...
    wallMesh2_1.rotation.y = Math.PI/2;
    wallMesh2_1.position.set(x, 2.5, z+7.3);
    elements.add(wallMesh2_1);

    const wallMesh2_2 = new THREE.Mesh(
        new THREE.PlaneGeometry(1.8, 2),
        wallMat
    );
    wallMesh2_2.rotation.y = Math.PI/2;
    wallMesh2_2.position.set(x, 4, z+1.85);
    elements.add(wallMesh2_2);

    const wallMesh2_3 = new THREE.Mesh(
        new THREE.PlaneGeometry(1, 5),
        wallMat
    );
    wallMesh2_3.rotation.y = Math.PI/2;
    wallMesh2_3.position.set(x, 2.5, z+0.5);
    elements.add(wallMesh2_3);
    ...
    makeInstance('/assets/models/Projector.glb').then((projector) => {
        projector.position.set(x+1.5,4,z+8);
        projector.scale.set(1,1,1);
        projector.rotation.y = -Math.PI*7/8;
    ...
    for (let j=0; j<4; j++) {
        for (let i=0; i<5; i++) {
            ...
            makeInstance('/assets/models/Table.glb').then((table) => {
                table.position.set(x+3+j*1.85,0,z+11-i*1.5);
                table.scale.set(0.08,0.18,0.08);
                elements.add(table);
            });

            makeInstance('/assets/models/StandChair.glb').then((chair) => {
                chair.position.set(x+1-j*1.85,0,z-2.7-i*1.5);
        ...
        makeInstance('/assets/models/Table.glb').then((table) => {
            table.position.set(x+3+j*1.85,0,z+11-10);
            table.scale.set(0.08,0.18,0.08);
            elements.add(table);
        });

        makeInstance('/assets/models/StandChair.glb').then((chair) => {
            chair.position.set(x+1-j*1.85,0,z-2.7-10);
            chair.scale.set(0.04,0.03,0.04);
            chair.rotation.y=-Math.PI/2;
            elements.add(chair);
        });
