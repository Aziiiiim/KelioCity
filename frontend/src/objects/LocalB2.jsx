...
// Bigger local used in floor B2
export function createLocalB2() {
    
    const elements = new THREE.Group();
    elements.userData.kind = "room";
    elements.userData.roomType = "LocalB2";

    // Wall and floor
    const floorGeo = new THREE.PlaneGeometry(6, 8.49);
    const floorMat = new THREE.MeshBasicMaterial( { color: 0x2F4680 } );
    const floorMesh = new THREE.Mesh(floorGeo, floorMat);
    floorMesh.rotation.x = Math.PI * -.5;
    floorMesh.position.set(3,0.01,4.245)
    elements.add(floorMesh);

    const wallGeo1 = new THREE.PlaneGeometry(6, 4);
    const wallGeo2 = new THREE.PlaneGeometry(8.49, 4);
    ...
    wallMesh2.position.set(5.99,2,4.245);
    ...
    makeInstance('/assets/models/Cabin.glb')
        .then((cabin) => {
        cabin.position.set(0.45, 1, 3.95);
        cabin.scale.set(3, 2.5, 2);
    ...
    // Standing PC
    makeInstance('/assets/models/StandingPC.glb')
        .then((pc) => {
        pc.position.set(5.3, 1.3, 7.7);
        pc.scale.set(2.5, 2.5, 2.5);
    ...
    // Power Box 1
    makeInstance('/assets/models/PowerBox.glb')
        .then((pb) => {
        pb.position.set(0.5, 0, 7);
        pb.scale.set(4, 3.5, 3.5);
    ...
    // Rack 1
    makeInstance('/assets/models/ServerRack.glb')
        .then((pb) => {
        pb.position.set(3, 0, 3.5);
        pb.scale.set(0.8, 0.6, 0.8);
        pb.rotation.y = Math.PI/2;
        elements.add(pb);
        })
        .catch(console.error);
    ...
    makeInstance('/assets/models/containerShelf.glb')
        .then((shelf) => {
        shelf.position.set(5.6, 1.51, 2.9);
    ...
    makeInstance('/assets/models/messageBoard.glb')
        .then((board) => {
        board.position.set(3, 1.21, 0.1);
        board.scale.set(0.02, 0.03, 0.03);
